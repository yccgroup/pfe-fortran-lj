MODULE MODFBCLJ
  ...
  ! Define FBCLJ type (same type name as PBCLJ, change via USE MODULE, keep the code separated while compiling)
  TYPE :: LJ
      INTEGER :: natoms
  ...
  CONTAINS
    ...
    ! generate particles' coordinates
    ...
    SUBROUTINE move(self,x,stepsize) 
      ...
      ! reject the move if the particle runs outside the box
      DO i =1, 3
        IF (pos(i) > self%L .OR. pos(i) < 0) THEN
          pos(i) = x(i)
        END IF
      END DO
